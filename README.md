...
**Resuming a conversation.** Entries recorded with a session id show a **Resume in chat** button. It reopens the chat view with the past output as context and reconnects to the stored CLI session — the model retains the full conversation — so you can simply type your next message to continue where you left off. Further turns keep updating the same history entry. (Resume needs the CLI's stored session under `~/.claude/projects/`; if it has been cleaned up, the resumed chat reports an error.)
