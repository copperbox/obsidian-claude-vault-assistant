...
**Context-window gauge.** A small ring next to the status badge fills clockwise as the conversation consumes the model's context window. It updates live from each assistant message while a turn runs and syncs to the CLI's exact count when the turn ends; hover it for the percentage and token counts. It turns red past 80%, a hint to start a **New chat** soon.
